...
The current empirical calibration uses a 10.7 MHz intermediate-frequency constant:
...
### 2. Convert samples to four logic bits

CSV values are converted into a packed sample byte using columns 0 through 3. For `.sr`, each stored byte is already packed by sigrok.
...
At that point it appends the current DATA level to the 24-bit buffer.
...
- Falling-edge DATA sampling
- 24-bit, three-byte transfers
- Fixed control frames `E3 4C 88`
- Frequency marker `54`
...
6. A warning when the final frame is close to or outside the configured frequency range.

## Reproduction Checklist

To reproduce a documented result:

1. Connect analyzer ground to IC701 ground.
2. Probe CE, DIN, DATA, and CLK in that order.
3. Capture at 8 MHz or higher; use 16 MHz for confirmation.
4. Tune to a known display frequency and allow the transfer to complete.
...
8. Compare the bus result with the display and note any capture that continued after the display reading.
