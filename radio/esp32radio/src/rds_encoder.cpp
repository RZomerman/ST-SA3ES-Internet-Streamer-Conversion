...
RdsGroup encodeGroup(const std::array<uint16_t, 4>& words,
  ...
  for (std::size_t block = 0; block < 4; ++block) {
    ...
    for (int bit = 25; bit >= 0; --bit) {
      result.bits[out++] = static_cast<uint8_t>((result.blocks[block] >> bit) & 1u);
    }
  }
  return result;
}
...
void Encoder::rebuildPaddedRadioText() {
  paddedRt_.fill(' ');
  const std::size_t length = std::min<std::size_t>(station_.radioText.size(), 64);
  std::copy_n(station_.radioText.begin(), length, paddedRt_.begin());
  if (length < paddedRt_.size()) paddedRt_[length] = '\r';
}

RdsGroup Encoder::makeGroup0A(uint8_t segment) {
  segment &= 0x03;
  uint16_t blockB = 0;
  blockB |= static_cast<uint16_t>(station_.tp) << 10;
  blockB |= static_cast<uint16_t>(station_.pty & 0x1F) << 5;
  blockB |= static_cast<uint16_t>(station_.ta) << 4;
  blockB |= static_cast<uint16_t>(station_.music) << 3;
  if (segment == 3) blockB |= 1u << 2;
  blockB |= segment;
  ...
  return encodeGroup({station_.pi, blockB, kRadio538Af[segment], blockD});
}

RdsGroup Encoder::makeGroup2A(uint8_t segment) {
  segment &= 0x0F;
  uint16_t blockB = 0x2000;
  blockB |= static_cast<uint16_t>(station_.tp) << 10;
  blockB |= static_cast<uint16_t>(station_.pty & 0x1F) << 5;
  blockB |= static_cast<uint16_t>(textAB_) << 4;
  blockB |= segment;
  ...
  return encodeGroup({station_.pi, blockB, blockC, blockD});
}

RdsGroup Encoder::nextGroup() {
  static constexpr std::array<uint8_t, 8> kSchedule{0, 2, 0, 0, 2, 0, 2, 0};
