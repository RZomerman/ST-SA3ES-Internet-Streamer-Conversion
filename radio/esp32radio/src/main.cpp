...
void        handle_stations_get  ( AsyncWebServerRequest *request ) ;
void        handle_stations_post ( AsyncWebServerRequest *request ) ;
void        handle_now_playing   ( AsyncWebServerRequest *request ) ;
void        loadStationMap() ;
void        saveStationMap() ;
...
HardwareSerial       controllerUart ( 1 ) ;              // UART link from the controller ESP
...
String               icyurl ;                            // Icecast station homepage
...
int32_t              activeStationKHz = -1 ;
bool                 activeStationMapped = false ;
bool                 controllerReady = false ;

void updateRdsMetadata ( const char* title ) ;
void myQueueSend ( QueueHandle_t q, const void* msg, int waittime = 0 ) ;

enum class FrequencyParseResult { VALID, MALFORMED, UNSUPPORTED } ;

FrequencyParseResult parseFrequency ( const char* text, int32_t& rfKHz )
{
  const size_t length = strlen ( text ) ;
  if ( ( length != 5 && length != 6 ) || text[length - 3] != '.' )
  {
    return FrequencyParseResult::MALFORMED ;
  }
  int32_t whole = 0 ;
  for ( size_t index = 0 ; index < length - 3 ; ++index )
  {
    if ( text[index] < '0' || text[index] > '9' ) return FrequencyParseResult::MALFORMED ;
    whole = whole * 10 + text[index] - '0' ;
  }
  if ( text[length - 2] < '0' || text[length - 2] > '9' ||
       text[length - 1] < '0' || text[length - 1] > '9' )
  {
    return FrequencyParseResult::MALFORMED ;
  }
  const int32_t hundredths = ( text[length - 2] - '0' ) * 10 + text[length - 1] - '0' ;
  rfKHz = whole * 1000 + hundredths * 10 ;
  if ( rfKHz < 76000 || rfKHz > 108000 || rfKHz % 50 != 0 )
  {
    return FrequencyParseResult::UNSUPPORTED ;
  }
  return FrequencyParseResult::VALID ;
}

String formatFrequency ( int32_t rfKHz )
{
  char value[8] ;
  snprintf ( value, sizeof(value), "%ld.%02ld",
             static_cast<long> ( rfKHz / 1000 ),
             static_cast<long> ( ( rfKHz % 1000 ) / 10 ) ) ;
  return String ( value ) ;
}

bool selectStation ( int32_t rfKHz, const sony_tuner::StationMapEntry* station,
                     const char* source )
{
  const bool mapped = station != nullptr ;
  if ( rfKHz == activeStationKHz && mapped == activeStationMapped ) return false ;

  activeStationKHz = rfKHz ;
  activeStationMapped = mapped ;
  sony_tuner::setPlaybackActive ( false ) ;
  icystreamtitle = "" ;
  icygenre = "" ;
  icydescription = "" ;
  icyurl = "" ;
  if ( mapped )
  {
    presetinfo.host = station->streamUrl ;
    presetinfo.hsym = station->name ;
    presetinfo.station_state = ST_STATION ;
    icyname = station->name ;
    updateRdsMetadata ( "" ) ;
    ESP_LOGI ( TAG, "%s selected %s MHz: %s", source,
               formatFrequency ( rfKHz ).c_str(), station->name ) ;
  ...
  else
  {
    ...
    presetinfo.host = "" ;
    presetinfo.hsym = "" ;
    rds::submitMetadata ( "        ", "", DEFAULT_RDS_PTY ) ;
    myQueueSend ( radioqueue, &stopcmd ) ;
    ESP_LOGI ( TAG, "%s selected unmapped frequency %s MHz", source,
               formatFrequency ( rfKHz ).c_str() ) ;
  }
  if ( controllerReady )                                 // Report our IP on every station change
  {
    controllerUart.printf ( "IP: %s\n", ipaddress.c_str() ) ;
  }
  return true ;
}
...
void updateRdsMetadata ( const char* title = nullptr )
{
  ...
  rds::submitMetadata ( station.c_str(), radioText, DEFAULT_RDS_PTY ) ;
}
...
//**************************************************************************************************
void myQueueSend ( QueueHandle_t q, const void* msg, int waittime )
{
...
void processSonyTuner()
{
  static bool lastPlaybackActive = false ;
  sony_tuner::StationSelection selection ;

  if ( sony_tuner::takeStationSelection ( selection ) )
  {
    lastPlaybackActive = false ;
    selectStation ( selection.rfKHz,
                    selection.mapped ? &selection.station : nullptr,
                    "Sony" ) ;
    return ;
  }

  const bool playbackActive = playingstat != 0 ;
  if ( playbackActive != lastPlaybackActive )
  {
    lastPlaybackActive = playbackActive ;
    sony_tuner::setPlaybackActive ( playbackActive ) ;
  }
}
...
void processControllerUart()
{
  ...
  static int32_t pendingKHz = 0 ;
  static uint32_t pendingSince = 0 ;

  while ( controllerUart.available() )
  {
    const char received = static_cast<char> ( controllerUart.read() ) ;
    ...
    if ( received != '\n' )
    {
      ...
      else line[lineLength++] = received ;
      continue ;
    }
    if ( discarding )
    {
      controllerUart.print ( "ERR LINE_TOO_LONG\n" ) ;
      discarding = false ;
      lineLength = 0 ;
      continue ;
    }
    line[lineLength] = '\0' ;
    lineLength = 0 ;
    constexpr char prefix[] = "FREQ_MHZ=" ;
    if ( strncmp ( line, prefix, sizeof(prefix) - 1 ) != 0 )
    {
      controllerUart.print ( "ERR MALFORMED\n" ) ;
      continue ;
    }
    int32_t rfKHz = 0 ;
    const FrequencyParseResult result = parseFrequency ( line + sizeof(prefix) - 1, rfKHz ) ;
    if ( result == FrequencyParseResult::MALFORMED )
    {
      controllerUart.print ( "ERR MALFORMED\n" ) ;
      continue ;
    }
    if ( result == FrequencyParseResult::UNSUPPORTED )
    {
      controllerUart.print ( "ERR UNSUPPORTED\n" ) ;
      continue ;
    }
    controllerUart.printf ( "IP: %s\n", ipaddress.c_str() ) ;   // Reply immediately on any valid command
    if ( rfKHz == activeStationKHz && activeStationMapped )
    {
      controllerUart.printf ( "OK FREQ_MHZ=%s\n", formatFrequency ( rfKHz ).c_str() ) ;
      continue ;
    }
    if ( pending )
    {
      if ( rfKHz == pendingKHz )
      {
        controllerUart.print ( "ERR PENDING\n" ) ;
        continue ;
      }
      controllerUart.print ( "ERR SUPERSEDED\n" ) ;
    }
    pending = true ;
    pendingKHz = rfKHz ;
    pendingSince = millis() ;
  ...
  if ( pending && millis() - pendingSince >= 2000 )
  {
    pending = false ;
    sony_tuner::StationMapEntry station{} ;
    if ( !sony_tuner::stationForFrequency ( pendingKHz, station ) )
    {
      controllerUart.print ( "ERR UNMAPPED\n" ) ;
      return ;
    }
    selectStation ( pendingKHz, &station, "Controller" ) ;
    controllerUart.printf ( "OK FREQ_MHZ=%s\n", formatFrequency ( pendingKHz ).c_str() ) ;
  }
}

String controllerLineValue ( const String& input )
{
  String value = input ;
  value.replace ( "\r", " " ) ;
  value.replace ( "\n", " " ) ;
...
void sendControllerStatus()
{
  ...
  static uint32_t previousCheck = 0 ;
  static String previousPlaybackState ;
  static int previousBitrate = -1 ;
  static String previousStation ;
  static String previousNowPlaying ;
  static String previousGenre ;
  static String previousStreamUrl ;

  if ( !controllerReady ) return ;
  if ( initialized && millis() - previousCheck < 100 ) return ;
  previousCheck = millis() ;
  const String playbackState = playingstat != 0 ? "TRUE" :
                               ( activeStationMapped && presetinfo.host.length() ? "SEARCH" : "IDLE" ) ;
  const int currentBitrate = mbitrate > 0 ? mbitrate : bitrate ;
  const String station = controllerLineValue ( icyname.length() ? icyname : presetinfo.hsym ) ;
  const String nowPlaying = controllerLineValue ( icystreamtitle ) ;
  const String genre = controllerLineValue ( icygenre ) ;
  const String streamUrl = controllerLineValue ( presetinfo.host ) ;

  if ( !initialized || station != previousStation )
  {
    controllerUart.printf ( "STATION: %s\n", station.c_str() ) ;
    previousStation = station ;
  }
  if ( !initialized || nowPlaying != previousNowPlaying )
  {
    controllerUart.printf ( "NOW_PLAYING: %s\n", nowPlaying.c_str() ) ;
    previousNowPlaying = nowPlaying ;
  }
  if ( !initialized || genre != previousGenre )
  {
    controllerUart.printf ( "GENRE: %s\n", genre.c_str() ) ;
    previousGenre = genre ;
  }
  if ( !initialized || currentBitrate != previousBitrate )
  {
    controllerUart.printf ( "BITRATE: %d KBPS\n", currentBitrate ) ;
    previousBitrate = currentBitrate ;
  }
  if ( !initialized || streamUrl != previousStreamUrl )
  {
    controllerUart.printf ( "STREAM_URL: %s\n", streamUrl.c_str() ) ;
    previousStreamUrl = streamUrl ;
  }
  if ( !initialized || playbackState != previousPlaybackState )
  {
    controllerUart.printf ( "PLAYING: %s\n", playbackState.c_str() ) ;
...
void setup()
{
  ...
  digitalWrite ( CONFIG_RADIO_CONTROLLER_AUDIO_RDY_GPIO, LOW ) ;
  pinMode ( CONFIG_RADIO_CONTROLLER_AUDIO_RDY_GPIO, OUTPUT ) ;
  maintask = xTaskGetCurrentTaskHandle() ;                // My taskhandle
  controllerUart.begin ( 115200, SERIAL_8N1,
                         CONFIG_RADIO_CONTROLLER_UART_RX_GPIO,
                         CONFIG_RADIO_CONTROLLER_UART_TX_GPIO ) ;
  const bool sonyTunerStarted = sony_tuner::begin() ;
  ...
  if ( !sonyTunerStarted )
  {
    ESP_LOGE ( TAG, "Sony tuner control initialization failed" ) ;
  }
  ...
  loadStationMap() ;                                     // Restore web-managed station map
  ...
  readIOprefs() ;                                        // Read pins used for SPI, TFT, VS1053, IR,
                                                         // Rotary encoder
  ini_block.i2s_spdif_pin = SPDIF_OUTPUT_PIN ;
  reservepin ( SPDIF_OUTPUT_PIN ) ;
  reservepin ( SONY_TUNER_CE_PIN ) ;
  reservepin ( SONY_TUNER_CLK_PIN ) ;
  reservepin ( SONY_TUNER_DATA_PIN ) ;
  reservepin ( SONY_TUNER_DIN_PIN ) ;
  reservepin ( SONY_TUNER_MUTE_PIN ) ;
  reservepin ( SONY_TUNER_AST_PIN ) ;
  reservepin ( SONY_TUNER_ST_PIN ) ;
  reservepin ( SONY_TUNER_SIG_PIN ) ;
  reservepin ( CONFIG_RADIO_CONTROLLER_UART_RX_GPIO ) ;
  reservepin ( CONFIG_RADIO_CONTROLLER_UART_TX_GPIO ) ;
  reservepin ( CONFIG_RADIO_CONTROLLER_AUDIO_RDY_GPIO ) ;
  #ifdef NO_LOCAL_CONTROLS
    ini_block.ir_pin = -1 ;
    ini_block.enc_clk_pin = -1 ;
    ini_block.enc_dt_pin = -1 ;
    ini_block.enc_sw_pin = -1 ;
    ini_block.enc_up_pin = -1 ;
    ini_block.enc_dwn_pin = -1 ;
  #else
  ...
  ini_block.reqvol = DEFAULT_VOLUME ;                    // Nonzero fixed startup volume
  ...
  cmdserver.on ( "/stations", HTTP_GET, handle_stations_get ) ;
  cmdserver.on ( "/stations", HTTP_POST, handle_stations_post ) ;
  cmdserver.on ( "/api/now-playing", HTTP_GET, handle_now_playing ) ;
  ...
  xTaskCreatePinnedToCore (
    ...
    1 ) ;                                                 // Run with radio application on CPU 1
  ...
  tftset ( 0, NAME ) ;                                    // Set screen segment text top line
  presetinfo.station_state = ST_STATION ;                 // Sony frequency selects the station
  presetinfo.preset = 0 ;
  presetinfo.highest_preset = 0 ;
  presetinfo.host = "" ;
  presetinfo.hsym = "" ;
  icyname = "" ;
  rds::submitMetadata ( "        ", "", DEFAULT_RDS_PTY ) ;
  ...
  controllerUart.flush() ;
  digitalWrite ( CONFIG_RADIO_CONTROLLER_AUDIO_RDY_GPIO, HIGH ) ;
}
...
void writeprefs ( AsyncWebServerRequest *request )
{
  ...
  saveStationMap() ;                                 // Preserve the independently managed station map
  //timerAlarmEnable ( timer ) ;                  // Enable the timer
...
String jsonEscape ( const String& input )
{
  ...
  for ( size_t index = 0 ; index < input.length() ; ++index )
  {
    ...
    else if ( static_cast<uint8_t> ( value ) >= 0x20 ) escaped += value ;
  }
  return escaped ;
}
...
void handle_now_playing ( AsyncWebServerRequest *request )
{
  String json = "{\"playing\":" + String ( playingstat != 0 ? "true" : "false" ) +
                ",\"frequency_mhz\":" ;
  if ( activeStationKHz >= 0 ) json += "\"" + formatFrequency ( activeStationKHz ) + "\"" ;
  else json += "null" ;
  json += ",\"station\":\"" + jsonEscape ( icyname.length() ? icyname : presetinfo.hsym ) +
          "\",\"now_playing\":\"" + jsonEscape ( icystreamtitle ) +
          "\",\"stream_url\":\"" + jsonEscape ( presetinfo.host ) +
          "\",\"station_url\":\"" + jsonEscape ( icyurl ) +
          "\",\"genre\":\"" + jsonEscape ( icygenre ) +
          "\",\"description\":\"" + jsonEscape ( icydescription ) +
          "\",\"content_type\":\"" + jsonEscape ( audio_ct ) +
          "\",\"advertised_bitrate_kbps\":" + String ( bitrate ) +
          ",\"measured_bitrate_kbps\":" + String ( mbitrate ) +
          ",\"metadata_interval_bytes\":" + String ( metaint ) + "}" ;
...
{
  char key[8] ;
  for ( size_t index = 0 ; index < sony_tuner::kMaximumStations ; ++index )
  {
    snprintf ( key, sizeof(key), "stf%02u", index ) ; nvs_erase_key ( nvshandle, key ) ;
    snprintf ( key, sizeof(key), "stn%02u", index ) ; nvs_erase_key ( nvshandle, key ) ;
    snprintf ( key, sizeof(key), "stu%02u", index ) ; nvs_erase_key ( nvshandle, key ) ;
  }
  nvssetstr ( "stct", String ( sony_tuner::stationCount() ) ) ;
  sony_tuner::StationMapEntry station{} ;
  for ( size_t index = 0 ; sony_tuner::stationAt ( index, station ) ; ++index )
  {
    snprintf ( key, sizeof(key), "stf%02u", index ) ; nvssetstr ( key, formatFrequency ( station.rfKHz ) ) ;
    snprintf ( key, sizeof(key), "stn%02u", index ) ; nvssetstr ( key, station.name ) ;
    snprintf ( key, sizeof(key), "stu%02u", index ) ; nvssetstr ( key, station.streamUrl ) ;
  }
  nvs_commit ( nvshandle ) ;
}
...
void loadStationMap()
{
  sony_tuner::StationMapEntry stations[sony_tuner::kMaximumStations]{} ;
  size_t count = 0 ;
  char key[8] ;
  if ( !nvssearch ( "stct" ) ) return ;
  const int expectedCount = nvsgetstr ( "stct" ).toInt() ;
  if ( expectedCount < 0 || expectedCount > static_cast<int> ( sony_tuner::kMaximumStations ) ) return ;
  for ( int index = 0 ; index < expectedCount ; ++index )
  {
    snprintf ( key, sizeof(key), "stf%02u", index ) ;
    if ( !nvssearch ( key ) ) break ;
    int32_t rfKHz = 0 ;
    const String frequency = nvsgetstr ( key ) ;
    if ( parseFrequency ( frequency.c_str(), rfKHz ) != FrequencyParseResult::VALID ) break ;
    snprintf ( key, sizeof(key), "stn%02u", index ) ;
    if ( !nvssearch ( key ) ) break ;
    const String name = nvsgetstr ( key ) ;
    snprintf ( key, sizeof(key), "stu%02u", index ) ;
    if ( !nvssearch ( key ) ) break ;
    const String url = nvsgetstr ( key ) ;
    if ( name.isEmpty() || url.isEmpty() ||
         name.length() >= sony_tuner::kStationNameSize ||
         url.length() >= sony_tuner::kStationUrlSize ) break ;
    stations[count].rfKHz = rfKHz ;
    strlcpy ( stations[count].name, name.c_str(), sizeof(stations[count].name) ) ;
    strlcpy ( stations[count].streamUrl, url.c_str(), sizeof(stations[count].streamUrl) ) ;
    ++count ;
  }
    if ( count != static_cast<size_t> ( expectedCount ) ||
      !sony_tuner::replaceStationMap ( stations, count ) )
  {
    ESP_LOGE ( TAG, "Stored station map is invalid" ) ;
  }
}
...
void handle_stations_get ( AsyncWebServerRequest *request )
{
  String page = F("<!doctype html><html><head><meta name=viewport content='width=device-width,initial-scale=1'>"
                  "<title>Radio stations</title><style>body{font-family:Verdana,sans-serif;margin:0;background:#f3f5f4;color:#17201c}"
                  "main{max-width:1100px;margin:auto;padding:32px 20px}h1{font-size:28px}table{width:100%;border-collapse:collapse;background:white}"
                  "th,td{padding:10px;border-bottom:1px solid #d7ddda;text-align:left}th{background:#163d32;color:white}"
                  "input{box-sizing:border-box;width:100%;padding:9px;border:1px solid #aeb9b4}th:first-child{width:18%}th:nth-child(2){width:28%}"
                  "button{margin-top:18px;padding:11px 20px;border:0;background:#c94f2c;color:white;font-weight:bold;cursor:pointer}"
                  ".now{background:#fff;border-top:4px solid #c94f2c;padding:18px;margin:0 0 28px}.nowhead{display:flex;align-items:center;gap:12px}"
                  ".status{font-size:12px;font-weight:bold;padding:5px 8px;background:#d9dedb}.status.live{background:#cce8d9;color:#07552d}"
                  ".title{font-size:24px;font-weight:bold;margin:12px 0 4px}.station{font-size:16px;color:#3f544b;margin:0 0 18px}"
                  "dl{display:grid;grid-template-columns:150px 1fr 150px 1fr;gap:8px 14px;margin:0}dt{font-weight:bold;color:#52645d}dd{margin:0;overflow-wrap:anywhere}"
                  "@media(max-width:650px){main{padding:18px 8px}th,td{padding:6px}input{padding:7px}.title{font-size:20px}dl{grid-template-columns:110px 1fr}}</style></head>"
                  "<body><main><h1>Radio station map</h1><section class=now><div class=nowhead><strong>Now playing</strong><span id=status class=status>OFFLINE</span></div>"
                  "<div id=title class=title>Nothing playing</div><div id=station class=station></div><dl>"
                  "<dt>Frequency</dt><dd id=frequency>-</dd><dt>Genre</dt><dd id=genre>-</dd>"
                  "<dt>Content type</dt><dd id=content>-</dd><dt>Bitrate</dt><dd id=bitrate>-</dd>"
                  "<dt>Description</dt><dd id=description>-</dd><dt>Station site</dt><dd id=site>-</dd>"
                  "<dt>Stream URL</dt><dd id=stream>-</dd><dt>Metadata interval</dt><dd id=interval>-</dd></dl></section>"
                  "<form method=post><table><thead><tr>"
                  "<th>Source frequency</th><th>Station name</th><th>Station URL</th></tr></thead><tbody>") ;
  for ( size_t index = 0 ; index < sony_tuner::kMaximumStations ; ++index )
  {
    sony_tuner::StationMapEntry station{} ;
    const bool present = sony_tuner::stationAt ( index, station ) ;
    page += "<tr><td><input name=freq" + String(index) + " inputmode=decimal value=\"" +
            ( present ? formatFrequency ( station.rfKHz ) : String() ) + "\"></td><td><input name=name" +
            String(index) + " maxlength=48 value=\"" + ( present ? htmlEscape ( station.name ) : String() ) +
            "\"></td><td><input name=url" + String(index) + " maxlength=144 value=\"" +
            ( present ? htmlEscape ( station.streamUrl ) : String() ) + "\"></td></tr>" ;
  }
  page += F("</tbody></table><button type=submit>Save station map</button></form>"
            "<script>const put=(id,v)=>document.getElementById(id).textContent=v||'-';"
            "async function refresh(){try{const r=await fetch('/api/now-playing',{cache:'no-store'});if(!r.ok)throw 0;const d=await r.json();"
            "const s=document.getElementById('status');s.textContent=d.playing?'LIVE':'IDLE';s.className='status'+(d.playing?' live':'');"
            "put('title',d.now_playing||'Nothing playing');put('station',d.station);put('frequency',d.frequency_mhz?d.frequency_mhz+' MHz':'-');"
            "put('genre',d.genre);put('content',d.content_type);put('bitrate',d.measured_bitrate_kbps?d.measured_bitrate_kbps+' kbps':(d.advertised_bitrate_kbps?d.advertised_bitrate_kbps+' kbps':'-'));"
            "put('description',d.description);put('site',d.station_url);put('stream',d.stream_url);put('interval',d.metadata_interval_bytes?d.metadata_interval_bytes+' bytes':'-');"
            "}catch(e){const s=document.getElementById('status');s.textContent='OFFLINE';s.className='status'}}refresh();setInterval(refresh,2000)</script>"
            "</main></body></html>") ;
...
void handle_stations_post ( AsyncWebServerRequest *request )
{
  ...
  for ( size_t index = 0 ; index < sony_tuner::kMaximumStations ; ++index )
  {
    ...
    const String frequency = request->arg ( "freq" + suffix ) ;
    const String name = request->arg ( "name" + suffix ) ;
    const String url = request->arg ( "url" + suffix ) ;
    if ( frequency.isEmpty() && name.isEmpty() && url.isEmpty() ) continue ;
    if ( frequency.isEmpty() || name.isEmpty() || url.isEmpty() ||
         name.length() >= sony_tuner::kStationNameSize ||
         url.length() >= sony_tuner::kStationUrlSize ||
         parseFrequency ( frequency.c_str(), stations[count].rfKHz ) != FrequencyParseResult::VALID )
    {
      request->send ( 400, "text/plain", "Each row needs a valid 76.00-108.00 MHz frequency, name, and URL." ) ;
      return ;
    }
    strlcpy ( stations[count].name, name.c_str(), sizeof(stations[count].name) ) ;
    strlcpy ( stations[count].streamUrl, url.c_str(), sizeof(stations[count].streamUrl) ) ;
  ...
  if ( !sony_tuner::replaceStationMap ( stations, count ) )
  {
    request->send ( 400, "text/plain", "Station frequencies must be unique 0.05 MHz channels." ) ;
    return ;
  }
  saveStationMap() ;
  request->redirect ( "/stations" ) ;
}
  ...
  if ( datamode == HEADER )                             // Handle next byte of MP3 header
  {
    ...
    else if ( b == '\n' )                               // Linefeed ?
    {
      ...
      if ( chkhdrline ( metalinebf ) )                  // Reasonable input?
      {
        ...
        {
          icygenre = decode_spec_chars ( metaline.substring ( 10 ) ) ;
          icygenre.trim() ;
        }
        else if ( lcml.startsWith ( "icy-description:" ) ||
                  lcml.startsWith ( "ice-description:" ) )
        {
          icydescription = decode_spec_chars ( metaline.substring ( 16 ) ) ;
          icydescription.trim() ;
        }
        else if ( lcml.startsWith ( "icy-url:" ) )
        {
          icyurl = metaline.substring ( 8 ) ;
          icyurl.trim() ;
        }
